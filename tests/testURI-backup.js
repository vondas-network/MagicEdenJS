...
function urlProcessor(uri) {
  ...
  try {
    ...
    //   let decodedtwo = decodeURI(decodedone);
    //   let decodedthree = decodeURI(decodedtwo);
    console.log("----DECODED------------------------------");
    ...
    let swapBackBracket = swapFrontBracket.replace("%22%3A%5B%7B%22", '":[{"');
    let swapCommaParanthesis = swapBackBracket.replace("%22%2C%22", '","');
    let swapColonParanthesis = swapCommaParanthesis.replace("%22%3A%22", '":"');
